...
const FinancialOverview = ({ estateFirmData, dashboardStats }) => {
  ...
  const loadFinancialData = async () => {
    ...
    try {
      ...
      // Fetch subscriptions (if not already covered by payments)
      ...
      // Payment method breakdown (requires payment_method column)
      ...
      // Mock monthly growth (can be calculated by comparing periods)
      const monthlyGrowth = 15.5; // Replace with real calculation later
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'completed': return <Badge bg="success"><CheckCircle size={12} className="me-1" /> Completed</Badge>;
      case 'pending': return <Badge bg="warning"><Clock size={12} className="me-1" /> Pending</Badge>;
      case 'failed': return <Badge bg="danger"><XCircle size={12} className="me-1" /> Failed</Badge>;
  ...
  // Use default values if props missing
 ...
 if (loading) {
  ...
  return (
    <div>
            ...
            <Card.Body>
              ...
              <div className="mt-2">
                <small className="text-muted">
                  {paymentStats.monthlyGrowth >= 0 ? '+' : ''}{paymentStats.monthlyGrowth}% from last month
                </small>
              </div>
            </Card.Body>
          </Card>
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="d-flex align-items-center">
              ...
              <div className="d-flex flex-wrap gap-2">
                <Badge bg={filter === 'all' ? 'primary' : 'light'} className="cursor-pointer" onClick={() => setFilter('all')}>All</Badge>
                <Badge bg={filter === 'subscription' ? 'primary' : 'light'} className="cursor-pointer" onClick={() => setFilter('subscription')}>Subscriptions</Badge>
                <Badge bg={filter === 'boost' ? 'primary' : 'light'} className="cursor-pointer" onClick={() => setFilter('boost')}>Boosts</Badge>
                <Badge bg={filter === 'earnings' ? 'primary' : 'light'} className="cursor-pointer" onClick={() => setFilter('earnings')}>Earnings</Badge>
                <Badge bg={filter === 'commission' ? 'primary' : 'light'} className="cursor-pointer" onClick={() => setFilter('commission')}>Commissions</Badge>
              </div>
            </div>
          ...
          {transactions.length === 0 ? (
            ...
            <div className="table-responsive">
              <Table hover className="mb-0">
                <thead>
                  <tr>
                    <th>Date</th><th>Transaction</th><th>Type</th><th>Amount</th><th>Status</th><th>Reference</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {transactions.map(t => (
                    <tr key={t.id}>
                      ...
                      <td>
                        <div className="fw-medium">{t.payment_type}</div>
                      ...
                      <td><code className="small">{t.reference}</code></td>
                      <td>
      ...
      {/* Earnings Breakdown & Payment Methods */}
        ...
        <Modal show={showTransactionDetails} onHide={() => setShowTransactionDetails(false)} size="lg">
          ...
          <Modal.Body>
            ...
            {selectedTransaction.metadata && (
              <Card className="bg-light"><Card.Body><pre>{JSON.stringify(selectedTransaction.metadata, null, 2)}</pre></Card.Body></Card>
            )}
          </Modal.Body>
        </Modal>
      )}
